...
namespace VRShooterKit
{
    ...
    {        
        ...
        [SerializeField] private Dictionary<Collider, VR_Grabbable> insideCollider = new Dictionary<Collider, VR_Grabbable>();
        ...
        private void Awake()
        {
            ...
            //dropzones in the collider mode, just trigger the dropzone if the object collides with this collider
            if (dropZoneMode == DropZoneMode.Collider)
            {
                SetupColliderBaseDropZone();
            }
        ...
        private void SetupColliderBaseDropZone()
        {
            for (int n = 0; n < dropZoneColliderArray.Length; n++)
            {
                TriggerCollisionListener listener = dropZoneColliderArray[n].gameObject.AddComponent<TriggerCollisionListener>();

                listener.OnTriggerEnterEvent += OnColliderTriggerEnter;
                listener.OnTriggerExitEvent += OnColliderTriggerExit;
            }
