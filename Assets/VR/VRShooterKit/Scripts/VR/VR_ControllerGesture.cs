...
namespace VRShooterKit
{
    ...
    public class VR_ControllerGesture : MonoBehaviour
    {
        ...
        {
            List<Quaternion> rotationHistory = controller.GetRotationHistorySample( sampleCount );

            //rotation history is no complete, this happens the first frames
            if (rotationHistory.Count < sampleCount)
            {                
                return null;
            }
